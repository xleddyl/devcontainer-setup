_dc_usage() {
  ...
  printf '\n  %s%s⬢  dev container%s\n\n' "$b" "$y" "$r"
  printf '  %s%s%s\n\n' "$b" "dc [-t] [-r] [-i] [-a <rule>]... <agent> [args...]" "$r"
  printf '  %sagents%s\n' "$d" "$r"
  ...
  printf '    %s%-18s%s allow one firewall rule for this session\n\n' "$c" "-a, --allow <rule>" "$r"
  printf '  %sexamples%s\n' "$d" "$r"
  ...
  printf '    %s%-18s%s git remotes open for this session\n\n' "$c" "dc -a git claude" "$r"
...
dc() {
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      -a | --allow)
        if [ $# -lt 2 ]; then
          echo "dc: $1 needs a rule name" >&2
          return 1
        fi
        allow_args+=(--allow "$2"); shift 2 ;;
